...
export const WeatherCard = () => {
	...
	if (weatherInfoArray !== []) {
		weatherCard = weatherInfoArray.map((weatherObj, index) => {
			return (
				<div className={`card ${weatherObj.status}`} key={index}>
					<div className='close-icon' onClick={() => closeCard(index)}></div>
					<Header
						datetime={weatherObj.dateTime}
						locate={weatherObj.locate}
						weather={weatherObj.weather}
					/>

					<canvas className={`chart chart-${index}`}>
						<Chart
							weatherState={weatherObj.status}
					...
					<WeatherBlock
						...
						otherInfo={weatherObj.otherInfo}
						weatherState={weatherObj.status}
					/>
				</div>
			);
