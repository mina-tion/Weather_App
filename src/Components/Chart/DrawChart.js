export const drawChart = (color, color2, data, className) => {
    ...
    let yStart = (1 - (data[0] - dataMin) / (dataMax - dataMin)) * canvasRef.height;
    ctx.lineTo(0, yStart);
    
    let transformXY = function(i) {
        return [canvasRef.width * i / (data.length - 1), (0.85 - (data[i] - dataMin) / (dataMax - dataMin) * 0.7) * canvasRef.height]
    }
